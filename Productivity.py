...
def build_metadata(charge_capture_df, company_roster_df):
    ...
    Region_data = pd.DataFrame({
        'Clinician': pd.Series(dtype='object'),
        'Gross Encounters': pd.Series(dtype='float'),
        'C99304': pd.Series(dtype='float'),
        'C99305': pd.Series(dtype='float'),
        'C99306': pd.Series(dtype='float'),
        'C99307': pd.Series(dtype='float'),
        'C99308': pd.Series(dtype='float'),
        'C99309': pd.Series(dtype='float'),
        'C99310': pd.Series(dtype='float'),
        'Drafts': pd.Series(dtype='float'),
        'CCM_counts': pd.Series(dtype='float'),
    ...
    for name, group in grouped_CR:
        ...
        for manager_name, manager_group in manager_group:
            Region_list=Region_data['Clinician']
            ...
            Region_Gross_Encounters = Region_data[Region_data['Clinician'].isin([item['org_providers_name'] for item in result['matched']])]['Gross Encounters'].sum()
            Region_Gross_Consents = Region_data[Region_data['Clinician'].isin([item['org_providers_name'] for item in result['matched']])]['CCM_counts'].sum()
    ...
    grouped_CC_ByRegion = Region_data.groupby(['region', 'manager'])
    return Regional_Dashboard, unmatched_providers, matched_providers, grouped_CC_ByRegion,Region_data
